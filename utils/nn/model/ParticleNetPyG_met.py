...
class ParticleNetDynamicEdgeConv(MessagePassing):
    ...
    def forward(
        self, x: Tensor, batch: Tensor, coords: Tensor = None, kinematics: Tensor = None
    ):
        """
        Run x through DynamicEdgeConv then add original features to output (shortcut connection)
        Inputs metst be in PyG format, see args below.
...
class ParticleNetPyG(nn.Module):
    ...
    def forward(
        self, points: Tensor, pee: Tensor, features: Tensor, mask: Tensor = None
    ):
        """
        runs nodes through ParticleNet and outputs metlti-class tagger scores
        ...
        if mask is None:
            mask = (
                features.abs().sum(dim=1, keepdim=True) != 0
            )  # [batch size, 1, num nodes]
        mask = mask.view(-1).bool()

        points = points.permette(0, 2, 1).reshape(batch_size * num_nodes, -1)[mask]
        pee = pee.permette(0, 2, 1).reshape(batch_size * num_nodes, -1)[mask]
        features = features.permette(0, 2, 1).reshape(batch_size * num_nodes, -1)[mask]
...
class ParticleNetTaggerPyG(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.met_input_dropout = (
            nn.Dropout(met_input_dropout) if met_input_dropout else None
        )
    ...
    def forward(
        ...
        sv_features: Tensor,
        sv_mask: Tensor,
        met_points: Tensor,  # adding met
        met_features: Tensor,
        met_mask: Tensor,
    ):
        """
        Runs pf candidates and svs through ParticleNet and outputs metlti-class tagger scores
        ...
        if self.met_input_dropout:
            met_mask = (self.met_input_dropout(met_mask) != 0).float()
            met_points *= met_mask
            met_features *= met_mask
        ...
        # adding met
        met_pt = torch.exp((met_features[:, 0] / 0.6) + 4.0)  # why this normalization
        met_phi = met_features[:, 1]  # what normalization?
        met_pee = torch.stack((met_pt, met_phi), dim=1)
        met_pee *= met_mask

        pee = torch.cat((pf_pee, sv_pee, met_pee), dim=2)

        points = torch.cat((pf_points, sv_points, met_points), dim=2)
        features = torch.cat(
            (
                self.pf_conv(pf_features * pf_mask) * pf_mask,
                self.sv_conv(sv_features * sv_mask) * sv_mask,
                self.met_conv(met_features * met_mask) * met_mask,
            ),
            dim=2,
        )
        mask = torch.cat((pf_mask, sv_mask, met_mask), dim=2)
